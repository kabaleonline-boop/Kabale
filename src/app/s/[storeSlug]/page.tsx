...
export default function StorefrontPage() {
  ...
  if (config.whatsappNumber) {
    let cleanNum = config.whatsappNumber.replace(/\D/g, ''); // Strip all spaces, dashes, etc
    if (cleanNum.startsWith('0')) {
      cleanNum = '256' + cleanNum.substring(1); // Convert 07... to 2567...
    } else if (!cleanNum.startsWith('256')) {
      cleanNum = '256' + cleanNum; // Prepend 256 if missing
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* 2. Edge-to-Edge Bio & Contact */}
      <div className="bg-white border-b border-slate-200 py-6 px-4">
        <div className="max-w-5xl mx-auto">
          {config.description ? (
            <p className="text-slate-700 font-medium leading-relaxed mb-4 text-sm sm:text-base max-w-2xl">
              {config.description}
            </p>
          ) : (
            <p className="text-slate-400 font-medium leading-relaxed mb-4 text-sm italic max-w-2xl">
          ...
          {config.whatsappNumber && (
            <a 
              ...
              className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold py-2 px-6 rounded-xl hover:bg-emerald-100 transition-colors shadow-sm"
            >
      ...
      {isOwner && (
        <div className="bg-slate-100 py-4 px-4 border-b border-slate-200 flex flex-col items-center justify-center gap-2">
