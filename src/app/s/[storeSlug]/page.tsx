...
export default function StorefrontPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* 1. Dynamic Gradient Header (Matches Mobile Preview Style) */}
      <div className="pt-12 pb-20 px-4 transition-all duration-500 relative" style={gradientStyle}>
        <div className="max-w-4xl mx-auto flex items-center justify-between relative z-10">
          
          <div className="flex items-center gap-4 sm:gap-5">
            {config.logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={config.logoUrl} alt={config.storeName} className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-white/30 object-cover bg-white shadow-md shrink-0" />
            ) : (
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/20 flex items-center justify-center font-bold text-2xl sm:text-3xl shadow-md border-4 border-white/30 backdrop-blur-sm shrink-0">🏪</div>
            )}
          ...
          <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center shrink-0 shadow-sm backdrop-blur-sm text-white text-xl border border-white/10 transition-transform hover:scale-105 cursor-pointer">
            🛒
          </div>
        </div>
      </div>

      {/* 2. Overlapping Bio & Contact Card */}
      <div className="max-w-3xl mx-auto px-4 -mt-10 relative z-20">
        <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] text-center border border-slate-100 flex flex-col items-center">
          
          <div className="h-1.5 w-12 bg-slate-200 rounded-full mx-auto mb-4"></div>
          
          {config.description ? (
            <p className="text-slate-600 font-medium leading-relaxed mb-6 text-sm sm:text-base max-w-xl">
              {config.description}
            </p>
          ) : (
            <p className="text-slate-400 font-medium leading-relaxed mb-6 text-sm italic max-w-xl">
          ...
          {config.whatsappNumber && (
            <a 
              ...
              className="inline-flex items-center gap-2 bg-slate-50 border border-slate-200 text-slate-900 font-bold py-3 px-8 rounded-full hover:bg-emerald-50 hover:border-emerald-200 hover:text-emerald-700 transition-colors shadow-sm"
            >
      ...
      <div className="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        {/* 🚨 MOVED STORE OWNER BANNER */}
        {isOwner && (
          <div className="bg-slate-100 py-4 px-6 text-center border border-slate-200 rounded-2xl mb-10 shadow-sm">
            ...
            <span className="text-slate-500 text-xs font-medium block mt-2">
